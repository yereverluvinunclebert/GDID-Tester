...
Private Sub btnGenerate_Click()
    ...
    gsPreviousGDIDValue = GDID
        
    Call writeLogFile("Generated a GDID manually - " & GDID, Now())
    Call tmrGDIDTester_TimerCode
...
writeLogs_Error:
     ...
     MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure writeLogs of Form Form1"
End Sub

'---------------------------------------------------------------------------------------
' Procedure : regWriteExtendedPropertiesLid
' Author    : beededea
' Date      : 09/08/2026
' Purpose   :
'---------------------------------------------------------------------------------------
'
Private Sub regWriteExtendedPropertiesLid(ByVal auto As Boolean, ByVal newGDID As String)

    Dim bResult As Boolean
    
    On Error GoTo regWriteExtendedPropertiesLid_Error
...
regWriteExtendedPropertiesLid_Error:

     MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure regWriteExtendedPropertiesLid of Form Form1"
End Sub
...
Private Sub setFirstRunStatus()
    ...
    Else
        ...
        Call writeLogFile("The original GDID Key Value as created by Microsoft " & gsOriginalGDID) ' read from the registry.
...
Private Sub tmrGDIDTester_Timer()
    ...
    ' check to see if it the GDID has changed
    If fGDIDStatusChanged = True Then
    
        If GDID = gsOriginalGDID Then
            Call writeLogFile("GDID was reverted to Microsoft original " & GDID, CStr(nowValue))
        End If
...
'---------------------------------------------------------------------------------------
' Procedure : tmrGDIDTester_TimerCode
' Author    : beededea
' Date      : 26/09/2026
' Purpose   :
'---------------------------------------------------------------------------------------
'
Private Sub tmrGDIDTester_TimerCode()

    On Error GoTo tmrGDIDTester_TimerCode_Error

     ' generate a new 16 character GDID string
    GDID = generateGDID
    
    ' change the GDID in the primary location, ie. the LID
    Call modifyExtendedPropertiesLid(GDID)
    
    ' extract and replace any previously auto-generated sub keys with a matching ID
    Call ReplaceMatchingDeviceIds(gsPreviousGDIDValue, gsAutoGeneratedGDID)
    
    ' extract and replace the recently MS-modified sub keys that have the original GDID reverted
...
tmrGDIDTester_TimerCode_Error:

     MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure tmrGDIDTester_TimerCode of Form Form1"
End Sub
...
'---------------------------------------------------------------------------------------
'
Public Function generateGDID() As String
...
Private Function fGDIDStatusChanged() As Boolean
    ...
    'save the current GDID value priorit to rechecking
    gsPreviousGDIDValue = GDID
    nowValue = Now()
    
    GDID = readRegistryExtendedPropertiesLid
    
    ' if it has been made blank, simply move on
    If GDID <> "" Then
        ...
        If GDID = gsPreviousGDIDValue Then
            fGDIDStatusChanged = False
        Else
            If gsPreviousGDIDValue = "" Then gsPreviousGDIDValue = GDID
...
'---------------------------------------------------------------------------------------
'
Private Sub modifyExtendedPropertiesLid(ByVal newGDID As String)
